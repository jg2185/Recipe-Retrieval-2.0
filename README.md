...
We only test the English text data for ingredients input, but currently the website does not support multilanguage and audio file input. The user can run these two with command line in the terminal. We will fix them in the future with better technical support. 
...
Please download the data from link and put it into the data folder is the easiest method using this package. We have automatic data downloading command however it needs more effort for the computer to take.
